...
def render_path(render_poses, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        if gt_imgs is not None and render_factor==0:
            p = -10. * np.log10(np.mean(np.square(rgb.cpu().numpy() - gt_imgs[i]))) # mod0
...
def create_nerf(args):
    ...
    if args.use_viewdirs:
        if args.reduce_embedding==2: # use Nerfie embedding
            if args.no_DNeRF_viewdir: # no Nerfie embedding for viewdir
    ...
    model = NeRF(D=args.netdepth, W=args.netwidth,
                 ...
                 input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs).to(torch.device("cuda"))
    ...
    if (args.dataset_type != 'llff' and args.dataset_type != '7Scenes_ryan') or args.no_ndc:
        print('Not ndc!')
...
def render_rays(ray_batch,
    ...
    if not lindisp:
        z_vals = near * (1.-t_vals) + far * (t_vals) # depth
    else:
        z_vals = 1./(1./near * (1.-t_vals) + 1./far * (t_vals)) # disparity
    ...
    pts = rays_o[...,None,:] + rays_d[...,None,:] * z_vals[...,:,None] # [N_rays, N_samples, 3] # seems like within +/- pi
    if i_epoch>=0:
...
def config_parser():
    ...
    parser.add_argument("--pose_scale", type=float, default=1, help='manual tuned pose scale factor, matching COLMAP result')
    ...
    parser.add_argument("--smart_data_loader", action='store_true', help='use "smarter" dataset selection ')
...
def train():
    ...
    if args.dataset_type == 'llff':
        ...
        from run_nerf_7Scenes import train_7scene

        # returns train dataloader, val dataloader, hwf, i_split
        train_dl, val_dl, hwf, i_split, bds, render_poses, render_img = load_7Scenes_dataloader_NeRF(args)
        near = args.near_far[0]
        far = args.near_far[1]

        print('NEAR FAR', near, far)
        train_7scene(args, train_dl, val_dl, hwf, i_split, near, far, render_poses, render_img)
        return
    ...
    if args.render_test:
        ...
        i_test = np.concatenate((i_train, i_test)) # render whole dataset
    ...
    N_iters = 200000 + 1 # 100 imgs * 2000 epochs, thus 7 scenes should be 4000imgs * 2000epochs = 8000000
    print('Begin')
    print('TRAIN views are', i_train)
    print('TEST views are', i_test)
    print('VAL views are', i_val)
    ...
    for i in trange(start, N_iters):
        ...
        if i%args.i_testset==0 and i > 0:
            ...
            os.makedirs(testsavedir, exist_ok=True)
            print('test poses shape', poses[i_test].shape) # ([20,4,4])
            with torch.no_grad():
